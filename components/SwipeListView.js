...
export const makeId = (rowId, secId) => `${secId}${rowId}`;
...
export default class SwipeListView extends Component {
  ...
  closeOpenRow = (fn) => {
    const row = this._rows[this.openCellId];
  ...
  getRow = (rowId, secId) => this._rows[makeId(rowId, secId)]

  onRowOpen = (rowData, rowId, secId, rowMap = this._rows) => {
    const id = makeId(rowId, secId);
    ...
    if (!this.props.renderHiddenRow) {
      ...
      return (
        <SwipeRow
          ...
          ref={row => this._rows[makeId(rowId, secId)] = row}
